...
# HELP_FIND_BACK CONTROLLER, SESSIONS
...
#Swagger documentation for logout.
  path '/users/sign_out' do
    ...
    delete 'Destroy JWT token' do
      # create a user, and then a token associated with that user
      let(:user) { FactoryBot.create(:user) }
      let (:auth_header) {"Bearer "+Warden::JWTAuth::UserEncoder.new.call(user, :user,nil)[0]}
      tags 'Terminate API Session'
      consumes 'application/json'
      produces 'application/json'
      # tell swagger that authentication is needed for this API
      security [Bearer: {}]
